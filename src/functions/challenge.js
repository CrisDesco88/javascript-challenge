...
export const listPaddockManagersByName = () => {
  let orden = paddockManagers.map(item => item.name).sort()
  let items = orden.map(orden => paddockManagers.find(x => x.name === orden))
  let ruts = items.map(paddokM => paddokM.taxNumber)
  return ruts
}
...
export const sortPaddockTypeByTotalArea = () => {
  const paddocksAreas = paddocks.reduce((acumulator, actualValue) => {
    const exist = acumulator.find(
      element => element.paddockTypeId === actualValue.paddockTypeId
    )
    if (exist) {
      return acumulator.map(element => {
        if (element.paddockTypeId === actualValue.paddockTypeId) {
          return {
            ...element,
            area: element.area + actualValue.area
          }
        }
  ...
  const orderedAreas = paddocksAreas.sort((a, b) => {
    return b.area - a.area
  })
  const paddocksById = orderedAreas.map(orderedAreas =>
    paddockType.find(x => x.id === orderedAreas.paddockTypeId)
  )
  const names = paddocksById.map(paddockName => paddockName.name)
  return names
}
...
export const sortFarmManagerByAdminArea = () => {
  const paddocksAreas = paddocks.reduce((acumulator, actualValue) => {
    const exist = acumulator.find(
      element => element.paddockManagerId === actualValue.paddockManagerId
    )
    if (exist) {
      return acumulator.map(element => {
        if (element.paddockManagerId === actualValue.paddockManagerId) {
          return {
            ...element,
            area: element.area + actualValue.area
          }
  ...
  const paddocksById = orderedAreas.map(orderedAreas =>
    paddockManagers.find(x => x.id === orderedAreas.paddockManagerId)
  )
  const names = paddocksById.map(paddockName => paddockName.name)
  return names
}
...
export const biggestAvocadoFarms = () => {
  const paltosPaddocks = paddocks.filter(item => item.paddockTypeId === 1)
  const paltosPaddocksAreas = paltosPaddocks.reduce(
    (acumulator, actualValue) => {
      const exist = acumulator.find(
        element => element.farmId === actualValue.farmId
      )
      if (exist) {
        return acumulator.map(element => {
          if (element.farmId === actualValue.farmId) {
            return {
              ...element,
              area: element.area + actualValue.area
            }
  ...
  const moreThan20000 = paltosPaddocksAreas.filter(item => item.area > 20000)

  return moreThan20000
    .map(item => item.area)
    .sort((a, b) => {
      return b - a
    })
...
export const biggestCherriesManagers = () => {
  const cherry = paddocks.filter(
  item => item.paddockTypeId === 3 && item.farmId === 3
)

const cherryAreas = cherry.reduce((acumulator, actualValue) => {
  const exist = acumulator.find(
    element => element.paddockManagerId === actualValue.paddockManagerId
  )
  if (exist) {
...
const moreThan1000 = cherryAreas.filter(item => item.area > 1000)
const managgersById = moreThan1000.map(orderedAreas =>
  paddockManagers.find(x => x.id === orderedAreas.paddockManagerId)
)
const names = managgersById.map(paddockName => paddockName.name).sort()
return names
